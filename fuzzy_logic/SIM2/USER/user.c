...
boolean StepRobot(struct Robot *robot)
{
  pas++;
  DrawStep();
  if ((robot->IRSensor[0].DistanceValue +
  	   robot->IRSensor[1].DistanceValue + /* if there is a collision on the*/
       robot->IRSensor[2].DistanceValue > COLLISION_TH /* left side of the robot */
   	   > COLLISION_TH))
  {
    robot->Motor[LEFT].Value  =  TURN_SPEED;
    robot->Motor[RIGHT].Value = -TURN_SPEED;          /* turn right */
  }
  else if ((robot->IRSensor[3].DistanceValue + /* if there is a collision on the*/
            robot->IRSensor[4].DistanceValue +/* right side of the robot */
  ...
  if ((robot->IRSensor[6].DistanceValue +  /* collision in */
